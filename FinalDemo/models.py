...
'''
Node (id, name, type, latitude, longitude, ip, port)
'''
class Node(db.Model):
    __tablename__ = 'node'
    id = db.Column(db.Integer, primary_key=True)
    ...
    nodelatitude = db.Column(db.Float)
    nodelongitude = db.Column(db.Float)
    nodeip = db.Column(db.String(32))
    nodeport = db.Column(db.Integer)
    nodeactive = db.Column(db.Boolean, default=False)
    timestamp = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
